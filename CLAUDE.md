...
- **จับกลุ่มเหตุคัดออกจากเลขข้อในข้อความ** (`ข้อ 1.6` / `ข้อ 1.11` / `ข้อ 1.7`) เพราะ `ineligibleReason` เป็น free text ที่ SurveyForm ประกอบขึ้น (ข้อความอายุมีตัวเลขอายุปนอยู่ จับด้วย string ตรง ๆ ไม่ได้)
- แสดงผล 3 จุด — **ไม่ทำหน้าแยก** (เจ้าหน้าที่ไม่เข้าไปดู): ชิปสรุปเหตุคัดออก + `EligibilityTip` (ℹ️ อธิบายเกณฑ์) ใต้ KPI หน้าภาพรวม · เหตุผลใต้ป้าย "ไม่เข้าเกณฑ์" ในตาราง (prop `reason` ใน `SurveyRow`) · บล็อกแดงในหน้ารายตัว (มีอยู่เดิม) · คอลัมน์ "เหตุไม่เข้าเกณฑ์" ใน export (มีอยู่เดิม)
...
- ไล่เฉดเขียว 5 ขั้น · จังหวัดไม่มีข้อมูล = เทา · ปัก **ชื่อจังหวัด + จำนวน** กลาง polygon (divIcon `.hi-prov` ใน globals.css; ตัวหนังสือบนเฉดเข้มเป็นสีขาว) เฉพาะจังหวัดที่มีข้อมูล
  - ชื่อจะโชว์เมื่อ polygon กว้าง **≥ 26px บนจอ** เท่านั้น (วัดด้วย `latLngToContainerPoint` ของ NE/NW) — ระดับซูมทั้งประเทศจังหวัดใหญ่กว้างราว 30px+ ได้ชื่อ ส่วนกรุงเทพฯ/อยุธยา ~12-15px เหลือแค่ตัวเลข แล้วค่อยมีชื่อเมื่อซูมเข้า; effect ผูกกับ state `zoom` ที่อัปเดตจาก `zoomend`
  - hover สองทาง: ชี้บน polygon → tooltip ชื่อ+ตัวเลข และแถวในแผงอันดับสว่าง / ชี้ที่แถว → polygon ขึ้นขอบเข้ม (state `hovered` + `styleFor()`; ไฮไลต์แตะทีละ layer ไม่ setStyle ใหม่ทั้ง 77 จังหวัด)
  - **แผนที่ย่อบนภาพรวมไม่มีตัวหนังสือ** — เล็กเกินกว่าจะใส่ ตัวเลขอ่านจากกราฟแท่งรายจังหวัดที่อยู่ข้างกัน
